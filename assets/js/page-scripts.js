...
$(document).ready(function() {
    // Menyembunyikan pesan kilat setelah navigasi halaman
    ...
    $('#ctrl-pilihan_pesanan').on('change', function() {
        var pilihan_pesanan = $(this).val().trim();
        var harga_pesanan = $('#ctrl-harga_pesanan');
        var tarif_ppn = $('#ctrl-tarif_ppn');
        var durasi_pesanan = $('#ctrl-durasi_pesanan');
        var subtotal_harga = $('#ctrl-subtotal_harga');
        var prices = {
            'TOKO ONLINE': 50000,
            'RDM MADRASAH': 30000,
            'WEBSITE SEKOLAH': 15000,
			'WEBSITE USAHA': 9000
        };
        var formattedPrice = 'Rp. ' + (prices[pilihan_pesanan] || '').toLocaleString('id-ID') + ' / Bulan';
        
        if (prices[pilihan_pesanan]) {
            harga_pesanan.val(formattedPrice).css('font-weight', 'bold');
        } else {
            harga_pesanan.val('');
        }
        tarif_ppn.val('');
        ...
        if (pilihan_pesanan === '') {
            harga_pesanan.val('').css('font-weight', 'normal');
        }
    });

$('#ctrl-durasi_pesanan').on('change', function(){ 
    var durasi_pesanan = $(this).val();
    var harga_pesanan = $('#ctrl-harga_pesanan').val().replace(/[^\d]/g, ''); // Menghapus karakter non-numerik
    var tarif_ppn = 0.12;
    var subtotal_harga = durasi_pesanan * harga_pesanan;
    var ppn_harga = subtotal_harga * tarif_ppn; // Menghitung PPN
    var total_harga = subtotal_harga + ppn_harga;
    // Menyesuaikan subtotal dan tampilan khusus untuk 24 bulan
    if(durasi_pesanan == 24) {
        ppn_harga = 'Rp. ' + ppn_harga.toLocaleString('id-ID');
        $('#ctrl-tarif_ppn').val(ppn_harga).css({'font-weight': 'bold', 'color': 'red', 'text-decoration': 'line-through'});
        total_harga = subtotal_harga; // Total harga tanpa PPN
    } else if(ppn_harga) {
        ppn_harga = 'Rp. ' + ppn_harga.toLocaleString('id-ID');
        $('#ctrl-tarif_ppn').val(ppn_harga).css({'font-weight': 'bold', 'color': 'red', 'text-decoration': 'none'});
    } else {
        $('#ctrl-tarif_ppn').val('').css({'font-weight': 'normal', 'color': 'black', 'text-decoration': 'none'});
    }
    // Menampilkan total harga dalam format mata uang
    if(total_harga) {
        total_harga = 'Rp. ' + total_harga.toLocaleString('id-ID');
        $('#ctrl-subtotal_harga').val(total_harga).css('font-weight', 'bold');
    } else {
        $('#ctrl-subtotal_harga').val('').css('font-weight', 'normal');
    }
});
$('#ctrl-pilihan_pesanansc').on('change', function() {
    var pilihan_pesanansc = $(this).val().trim();
    var harga_pesanansc = $('#ctrl-harga_pesanansc');
    var tarif_ppnsc = $('#ctrl-tarif_ppnsc');
    var subtotal_hargasc = $('#ctrl-subtotal_hargasc');
    var durasi_pesanansc = $('#ctrl-durasi_pesanansc');
    var tarif_ppn_val = 0.10; 
    if (pilihan_pesanansc === 'CONTOH1') {
        var harga_val = 100000;
        harga_pesanansc.val('Rp. ' + harga_val.toLocaleString('id-ID')).css('font-weight', 'bold');
        durasi_pesanansc.val('Selamanya');
        var ppn_hargasc_val = harga_val * tarif_ppn_val;
        tarif_ppnsc.val('Rp. ' + ppn_hargasc_val.toLocaleString('id-ID')).css({'font-weight': 'bold', 'color': 'red'});
        var total_hargasc = harga_val + ppn_hargasc_val;
        subtotal_hargasc.val('Rp. ' + total_hargasc.toLocaleString('id-ID')).css('font-weight', 'bold');
    } 
    else if (pilihan_pesanansc === 'CONTOH2') {
        var harga_val = 200000;
        harga_pesanansc.val('Rp. ' + harga_val.toLocaleString('id-ID')).css('font-weight', 'bold');
        durasi_pesanansc.val('Selamanya');
        var ppn_hargasc_val = harga_val * tarif_ppn_val;
        tarif_ppnsc.val('Rp. ' + ppn_hargasc_val.toLocaleString('id-ID')).css({'font-weight': 'bold', 'color': 'red'});
        ...
        harga_pesanansc.val('').css('font-weight', 'normal');
        tarif_ppnsc.val('');
        durasi_pesanansc.val('');
        subtotal_hargasc.val('');
    }
});
});
/**
